...
sys.path.append('../pytorch_images/')
...
bank_L1_reg_rate = float(sys.argv[7])
...
controller = DiffController(model=PreActResNet18, bank_size=bank_size,
                        key_size=key_size, y_size=y_size)
controller.cuda()

training(controller, 150, initial_learning_rate,
         trainloader, testloader, if_ten_crop=if_ten_crop,
         test_device='gpu', loss_interval=3, accum_grad_num=2,
         fybl1lr=bank_L1_reg_rate, forget_option=False)
